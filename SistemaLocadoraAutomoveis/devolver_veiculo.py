...
class DevolveVeiculo(Toplevel):
    ...
    def __init__(self, master=None):
        ...
        self.botao_locar_veiculo = Button(self, text="Confirmar \ndevolução", width=10, height=4, bg='#baffc9', fg='black', font=(
            'Verdana 15 bold'), command=self.devolver)
        ...
        self.cod_cliente = Label(self, text="Codigo do Cliente:", bg='#c9c9ff', fg='white', font=(
            'Verdana 15 bold'))
        self.cod_cliente.place(x=80, y=70)
        self.cod_cliente_entry = Entry(self, width=20, font=(
            'Verdana 15 bold'))
        self.cod_cliente_entry.place(x=370, y=70)
        ...
        self.lista_locacoes.configure(
            yscrollcommand=self.scrollbar_locacoes.set)
        self.scrollbar_locacoes.configure(
            command=self.lista_locacoes.yview)
        self.scrollbar_locacoes.place(x=1293, y=370, relheight=0.5)
        ...
        self.view_command()
        self.lista_locacoes.bind(
            '<<ListboxSelect>>', self.selecionar_list_box_locacoes)
    ...
    def devolver(self):
        try:
            ...
            tkinter.messagebox.showinfo(
                'Aviso!', 'Erro ao apagar dados!')
        else:
            tkinter.messagebox.showinfo(
                'Aviso!', 'Devolução efetuada!')
            self.view_command()
            self.cod_cliente_entry.delete(0, END)
            self.id_veiculo_entry.delete(0, END)
            self.valor_locacao_entry.insert(0, self.selecionado[5])
    ...
    def view_command(self):
        ...
        try:
            ...
            for r in rows:
                self.lista_locacoes.insert(END, r)
        except Exception as e:
            print(e)

    def selecionar_list_box_locacoes(self, event):
        if self.lista_locacoes.curselection():
            indice = self.lista_locacoes.curselection()[0]
            self.selecionado = self.lista_locacoes.get(indice)
            self.id = self.selecionado[0]
            self.cod_cliente_entry.delete(0, END)
            self.cod_cliente_entry.insert(0, self.selecionado[1])
            self.id_veiculo_entry.delete(0, END)
            self.id_veiculo_entry.insert(0, self.selecionado[2])
