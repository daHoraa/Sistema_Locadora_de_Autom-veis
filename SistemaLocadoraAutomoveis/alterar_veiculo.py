...
class AlteraVeiculo(Toplevel):
    ...
    def __init__(self, master=None):
        ...
        self.title('Editar dados de veiculos')
        ...
        self.veiculo_box.bind('<<ListboxSelect>>', self.selecionar_list_box)
    ...
    def selecionar_list_box(self, event):
        if self.veiculo_box.curselection():
            self.clear_all()
            indice = self.veiculo_box.curselection()[0]
            self.selecionado = self.veiculo_box.get(indice)

            self.id = self.selecionado[0]
            self.marca_entry.insert(END,self.selecionado[1])
            self.modelo_entry.insert(END,self.selecionado[2])
            self.ano_entry.insert(END,self.selecionado[3])
            self.cor_entry.insert(END,self.selecionado[4])
            self.tanque_entry.insert(END,self.selecionado[5])
            self.combustivel_entry.insert(END,self.selecionado[6])
            self.consumo_cidade_entry.insert(END,self.selecionado[7])
            self.consumo_estrada_entry.insert(END,self.selecionado[8])
            self.tempo_0_100_entry.insert(END,self.selecionado[9])
            self.chassi_entry.insert(END,self.selecionado[10])
            self.placa_entry.insert(END,self.selecionado[11])
            self.tamanho_pneu_entry.insert(END,self.selecionado[12])
            self.som_entry.insert(END,self.selecionado[13])
            self.valor_diaria_entry.insert(END,self.selecionado[14])
    ...
    def get_items(self):
        ...
        if(self.veiculo.marca == '' or self.veiculo.modelo == '' or self.veiculo.ano == '' or self.veiculo.cor == '' or self.veiculo.tanque == '' or self.veiculo.combustivel == '' or self.veiculo.consumo_cidade == '' or self.veiculo.consumo_estrada == '' or self.veiculo.tempo_0_100 == '' or self.veiculo.chassi == '' or self.veiculo.placa == '' or self.veiculo.tamanho_pneu == '' or self.veiculo.som == '' or self.veiculo.valor_diaria == ''):
            ...
            try:
                ...
                try:
                    self.dao.update(self.veiculo, self.id)
                except Exception as e:
                    print(e)
                else:
                    tkinter.messagebox.showinfo(
