...
class AlteraCliente(Toplevel):
    ...
    def get_items(self):
        ...
        if(self.cliente.nome == '' or self.cliente.rg == '' or self.cliente.cpf == '' or self.cliente.email == '' or self.cliente.telefone == '' or self.cliente.nascimento == '' or self.cliente.estado_civil == '' or self.cliente.genero == '' or self.cliente.cep == '' or self.cliente.logradouro == '' or self.cliente.bairro == '' or self.cliente.numero_logradouro == '' or self.cliente.cidade == '' or self.cliente.estado == '' or self.cliente.complemento == '' or self.cliente.numero_cnh == '' or self.cliente.numero_registro_cnh == '' or self.cliente.data_validade_cnh == '' or self.cliente.uf_cnh == '' or self.cliente.contato_emergencial == '' or self.cliente.nome_contato_emergencial == ''):
            ...
            try:
                ...
                try:
                    self.dao.update(self.cliente,self.id)
                except Exception as e:
    ...
    def selecionar_list_box(self, event):
        ...
        if self.lista_clientes.curselection():
            ...
            self.nome_entry.insert(END,self.selecionado[1])
            self.rg_entry.insert(END,self.selecionado[2])
            self.cpf_entry.insert(END,self.selecionado[3])
            self.email_entry.insert(END,self.selecionado[4])
            self.telefone_entry.insert(END,self.selecionado[5])
            self.nascimento_entry.insert(END,self.selecionado[6])
            self.estado_civil_entry.insert(END,self.selecionado[7])
            self.genero_entry.insert(END,self.selecionado[8])
            self.cep_entry.insert(END,self.selecionado[9])
            self.logradouro_entry.insert(END,self.selecionado[10])
            self.bairro_entry.insert(END,self.selecionado[11])
            self.numero_logradouro_entry.insert(END,self.selecionado[12])
            self.cidade_entry.insert(END,self.selecionado[13])
            self.estado_entry.insert(END,self.selecionado[14])
            self.complemento_entry.insert(END,self.selecionado[15])
            self.numero_cnh_entry.insert(END,self.selecionado[16])
            self.numero_registro_cnh_entry.insert(END,self.selecionado[17])
            self.data_validade_cnh_entry.insert(END,self.selecionado[18])
            self.uf_cnh_entry.insert(END,self.selecionado[19])
            self.contato_emergencial_entry.insert(END,self.selecionado[20])
            self.nome_contato_emergencial_entry.insert(END,self.selecionado[21])
