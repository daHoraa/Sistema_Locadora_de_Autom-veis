...
class ClienteDAO:
    ...
    def view(self):
        resultado = None
        try:
            self.database.execute("SELECT * FROM clientes")
    ...
    def insert(self, cliente):
        ...
        try:
            self.database.execute("INSERT INTO clientes(nome, rg, cpf, email, telefone, nascimento, estado_civil, genero, cep, logradouro, bairro, numero_logradouro, cidade, estado, complemento, numero_cnh, numero_registro_cnh, data_validade_cnh,uf_cnh, contato_emergencial, nome_contato_emergencial) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
                                  (cliente.nome, cliente.rg, cliente.cpf, cliente.email,  cliente.telefone,  cliente.nascimento,  cliente.estado_civil,  cliente.genero,  cliente.cep,  cliente.logradouro,  cliente.bairro,  cliente.numero_logradouro,  cliente.cidade,  cliente.estado,  cliente.complemento,  cliente.numero_cnh,  cliente.numero_registro_cnh,  cliente.data_validade_cnh,  cliente.uf_cnh,  cliente.contato_emergencial,  cliente.nome_contato_emergencial))
            self.database.persist()
        except sqlite3.Error:
    ...
    def update(self, cliente, id):
        ...
        try:
            ...
            self.database.execute("UPDATE clientes SET nome=?, rg=?, cpf=?, email=?, telefone=?, nascimento=?, estado_civil=?, genero=?, cep=?, logradouro=?, bairro=?, numero_logradouro=?, cidade=?, estado=?, complemento=?, numero_cnh=?, numero_registro_cnh=?, data_validade_cnh=?, uf_cnh=?, contato_emergencial=?, nome_contato_emergencial =? WHERE id=?", (cliente.nome, cliente.rg, cliente.cpf, cliente.email,
                                                                                                                                                                                                                                                                                                                                                                     cliente.telefone,  cliente.nascimento,  cliente.estado_civil,  cliente.genero,  cliente.cep,  cliente.logradouro,  cliente.bairro,  cliente.numero_logradouro,  cliente.cidade,  cliente.estado,  cliente.complemento,  cliente.numero_cnh,  cliente.numero_registro_cnh,  cliente.data_validade_cnh,  cliente.uf_cnh,  cliente.contato_emergencial,  cliente.nome_contato_emergencial, id))
            self.database.persist()
        except sqlite3.Error:
    ...
    def delete(self, id):
        ...
        try:
            self.database.execute("DELETE FROM clientes WHERE id=?", (id, ))
            self.database.persist()
